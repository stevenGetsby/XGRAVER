#!/bin/bash
# Stage 3: Train per-block UDF with sparse flow matching

MODE=${1:-test}
echo "=========================================="
echo "Stage 3: Feats Training (mode=$MODE)"
echo "=========================================="

CONFIG_FILE="./configs/flow_matching/block_feats.json"

if [ "$MODE" = "test" ]; then
    OUTPUT_DIR="../ckpt/feats_test"
    LOAD_DIR="../ckpt/feats_test"
    rm -rf "$OUTPUT_DIR"
    mkdir -p "$OUTPUT_DIR"
    CKPT="latest"
elif [ "$MODE" = "train" ]; then
    export SKIP_SNAPSHOT=1
    OUTPUT_DIR="./ckpt/feats_train"
    LOAD_DIR="./ckpt/feats_train"
    CKPT="latest"
else

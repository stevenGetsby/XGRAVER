...
class DenseFlowModel(nn.Module):
        ...
        **kwargs,
        ...
        if share_mod:
            self.adaLN_modulation = nn.Sequential(
                nn.SiLU(),
                nn.Linear(model_channels, n_mod * model_channels, bias=True)
            )
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, cond: torch.Tensor) -> torch.Tensor:
        ...
        # RoPE needs 3D grid coordinates as indices
        rope_indices = self.grid_coords if self.pe_mode == "rope" else None
        ...
        for block in self.blocks:
            h = block(h, t_emb, cond, indices=rope_indices)
        h = h.type(x.dtype)
        shift, scale = self.final_adaLN(t_emb_orig.type(x.dtype)).chunk(2, dim=-1)

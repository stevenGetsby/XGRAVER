...
def init_render(engine='CYCLES', resolution=512, geo_mode=False, fast_mode=False, ultra_fast_mode=False):
    ...
    if engine == 'CYCLES':
        ...
        # 根据模式调整采样数
        if ultra_fast_mode:
            bpy.context.scene.cycles.samples = 4  # 超快速模式使用最少采样
            bpy.context.scene.cycles.use_adaptive_sampling = True
            bpy.context.scene.cycles.adaptive_threshold = 0.1
            bpy.context.scene.cycles.adaptive_min_samples = 2
        elif fast_mode:
            bpy.context.scene.cycles.samples = 8  # 快速模式使用较少采样
            bpy.context.scene.cycles.use_adaptive_sampling = True
            bpy.context.scene.cycles.adaptive_threshold = 0.08
        ...
        if ultra_fast_mode:
            ...
            bpy.context.scene.cycles.transmission_bounces = 2 if not geo_mode else 1
        else:
        ...
        # 简化设置
        bpy.context.scene.cycles.use_denoising = not (fast_mode or ultra_fast_mode)  # 快速模式关闭降噪
        bpy.context.scene.cycles.use_fast_gi = True  # 使用快速全局照明
        
        # 优化瓦片大小（GPU渲染）
        if ultra_fast_mode:
            bpy.context.scene.cycles.tile_size = 512  # 超快速模式使用更大的瓦片
        else:
        ...
        # EEVEE 特定设置 - 优化版本
        if ultra_fast_mode:
            bpy.context.scene.eevee.taa_render_samples = 4  # 超快速模式使用最少采样
            bpy.context.scene.eevee.use_bloom = False
            bpy.context.scene.eevee.use_ssr = False
            bpy.context.scene.eevee.use_gtao = False
            bpy.context.scene.eevee.use_soft_shadows = False
        elif fast_mode:
            bpy.context.scene.eevee.taa_render_samples = 8  # 快速模式使用较少采样
            bpy.context.scene.eevee.use_bloom = False
            bpy.context.scene.eevee.use_ssr = False
            bpy.context.scene.eevee.use_gtao = False
        else:
            bpy.context.scene.eevee.taa_render_samples = 64 if not geo_mode else 1
...
def normalize_scene() -> Tuple[float, Vector]:
    ...
    scale = 0.98 / max(bbox_max - bbox_min)
...
def main(arg):
    ...
    # Initialize context
    fast_mode = getattr(arg, 'fast_mode', False)
    ultra_fast_mode = getattr(arg, 'ultra_fast_mode', False)
    init_render(engine=arg.engine, resolution=arg.resolution, geo_mode=arg.geo_mode, fast_mode=fast_mode, ultra_fast_mode=ultra_fast_mode)

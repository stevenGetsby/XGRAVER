...
def _load_model_from_weight(config_path: str, weight_path: str,
                            model_key: str = 'denoiser',
                            device: str = 'cuda') -> torch.nn.Module:
    """从直接的权重文件加载模型 (如 HuggingFace 下载的 .pt 文件)."""
    cfg = _load_json(config_path)
    model_cfg = cfg.models[model_key]
    model = getattr(models, model_cfg.name)(**model_cfg.args)
...
@torch.no_grad()
def sample_coords(model, cond: torch.Tensor, *,
                  noise_scale: float = 1.0, cfg_strength: float = 3.0,
                  cfg_interval: Tuple[float, float] = (0.1, 1.0),
                  steps: int = 50, device: str = 'cuda',
                  threshold: float = 0.5) -> torch.Tensor:
    """Image cond → DenseFlowModel → 64³ occupancy → block coordinates [M, 3].

    Returns int32 tensor of active block coordinates.
    """
    # DenseFlowModel expects [B, C, D, D, D] with C=1 for occupancy
    noise = torch.randn(1, 1, BLOCK_GRID, BLOCK_GRID, BLOCK_GRID,
                        device=device) * noise_scale
    ...
    occ = (result.samples[0, 0].float() > threshold).cpu().numpy()  # [64, 64, 64]
    coords = np.argwhere(occ).astype(np.int32)  # [M, 3]
    return torch.from_numpy(coords).int()
...
def sample_mask(model, cond: torch.Tensor, coords: torch.Tensor, *,
               device: str = 'cuda',
               threshold: float = 0.5) -> torch.Tensor:
    """DirectMaskModel: coords + image cond → logits → sigmoid → binary mask.

    不使用 flow matching / noise / CFG, 直接一次前向预测.
    """
    ...
    dummy = SparseTensor(
        feats=torch.zeros(batch_coords.shape[0], model.token_dim, device=device),
        coords=batch_coords,
    )
...
def main():
    ...
    parser.add_argument('--coords_weight', type=str, default='',
                        help='Direct path to coords model weight file (.pt)')
    parser.add_argument('--coords_config', type=str, default='',
                        help='Config json for coords model')
    parser.add_argument('--mask_ckpt_dir', type=str, default='')
    parser.add_argument('--mask_weight', type=str, default='',
                        help='Direct path to mask model weight file (.pt)')
    parser.add_argument('--feats_ckpt_dir', type=str, default='')
    parser.add_argument('--feats_weight', type=str, default='',
                        help='Direct path to feats model weight file (.pt)')
    ...
    has_coords = bool(args.coords_weight)
    has_mask = bool(args.mask_ckpt_dir) or bool(args.mask_weight)
    has_feats = bool(args.feats_ckpt_dir) or bool(args.feats_weight)
    ...
    if has_mask:
        dirs['mask_only'] = os.path.join(args.output_dir, 'mask_only')
        os.makedirs(dirs['mask_only'], exist_ok=True)
    ...
    for seq_num, idx in enumerate(sample_indices, start=1):
        ...
        if has_mask:
            ...
            # 用预测 mask 应用到 GT feats 上渲染法线
            voxel_mask = _upsample_submask_to_voxel(pred_mask.cpu(), pred_mask_res)
            masked_feats = gt_feats.clone()
            masked_feats[voxel_mask.squeeze() < 0.5] = 1.0  # non-surface → UDF far
            _save_mesh_and_normal(
                coords_np, masked_feats.numpy().astype(np.float32),
                os.path.join(dirs['mask_only'], f'{seq_num}.ply'),
                os.path.join(dirs['mask_only'], f'{seq_num}_normal.jpg'),
            )
        ...
        if has_coords and has_mask and has_feats:
            ...
            print(f'    predicted {pred_coords.shape[0]} blocks (GT={n_blocks})')
            ...
            if pred_coords.shape[0] < 5:
                ...
                print(f'  [full_pipeline] stage 3: sampling feats ...')
                pred_feats = sample_feats(
                    feats_model, cond, pred_coords, submask_for_feats,
                    noise_scale=args.noise_scale_feats,
                    cfg_strength=args.cfg_strength,
                    cfg_interval=(args.cfg_interval_min, args.cfg_interval_max),
                    steps=args.steps, device=args.device,
                ...
                torch.cuda.empty_cache()
                _save_mesh_and_normal(
                    pred_coords_np, pred_feats.cpu().numpy().astype(np.float32),
                    os.path.join(dirs['full_pipeline'], f'{seq_num}.ply'),
                    os.path.join(dirs['full_pipeline'], f'{seq_num}_normal.jpg'),
                )
    ...
    for folder_key, folder_path in dirs.items():
        if folder_key == 'cond':
            ...
            img_files = sorted(
                [f for f in os.listdir(folder_path) if f.endswith('_normal.jpg')],
                key=lambda x: int(x.split('_')[0]),
            )
        if not img_files:
            continue
        total = len(img_files)
        n_grids = (total + grid_size - 1) // grid_size
        for gi in range(n_grids):
            batch = img_files[gi * grid_size : (gi + 1) * grid_size]
            imgs = []
            for fname in batch:
            ...
            grid = Image.new('RGB', (w * ncol, h * nrow), (0, 0, 0))
            for i, im in enumerate(imgs):
                r, c = divmod(i, ncol)
                grid.paste(im, (c * w, r * h))
            start_idx = gi * grid_size + 1
            end_idx = gi * grid_size + len(batch)
            grid_path = os.path.join(folder_path, f'grid_{start_idx:03d}_{end_idx:03d}.jpg')
            grid.save(grid_path, quality=90)
            print(f'  Grid saved: {grid_path} ({len(batch)} images)')

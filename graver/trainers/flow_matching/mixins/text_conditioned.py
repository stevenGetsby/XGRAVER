...
class TextConditionedMixin:
    ...
    def get_inference_cond(self, cond=None, img_cond=None, txt_cond=None, **kwargs):
        """
        Get the conditioning data for inference.
        - 支持文本接口(cond 或 txt_cond)
        - 兼容上游传入的 img_cond/txt_cond 形式（即便本 mixin只使用文本）
        """
        # 规范化文本来源：优先使用 txt_cond，其次 cond
        if txt_cond is not None:
            assert isinstance(txt_cond, list) and (len(txt_cond) == 0 or isinstance(txt_cond[0], str)), \
                "TextConditionedMixin expects txt_cond as list[str]"
            text_list = txt_cond
        else:
            assert cond is not None, "TextConditionedMixin requires text condition (cond or txt_cond)"
            assert isinstance(cond, list) and (len(cond) == 0 or isinstance(cond[0], str)), \
                "TextConditionedMixin expects cond as list[str]"
            text_list = cond

        text_tokens = self.encode_text(text_list)
        kwargs['neg_cond'] = self.text_cond_model['null_cond'].repeat(text_tokens.shape[0], 1, 1)

        # 直接返回统一的键，兼容上游调用
        return {'img_cond': img_cond, 'txt_cond': text_tokens, **kwargs}
